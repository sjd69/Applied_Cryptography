...
### Mechanism
We will utilize RSA to both authenticate and exchange keys. The server will generate a key pair, consisting of a public key and a private key. When the server creates a token, it will first stream the individual issuer, subject, and group information into a byte array, which can be of size up to 117-bits. We will then utilize MD5 to make a hash of the byte array. The result of that hash will be signed by the server. The server will then send the signed hash of token information to the user, along with a challenge which is encrypted with the user's public key. We can then verify that signature of the hash of the token to validate the user's token. 
